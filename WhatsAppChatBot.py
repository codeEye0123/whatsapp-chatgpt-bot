...
def send(msg):                      #Defining the send function
    print(">%s"%msg)
    p.typewrite("CodeEye.Bot: ")            #Type Tridib.Bot: before original message
    ...
    try:                            #Try and expect any error
        ...
        if cb != lastwrd:           #if the very last message and the newly copied message is same, ignore it as there's no new message
            ...
            if "your name" in cb:
                counter3 = counter3+1
                if counter3 <=1:
                    send("My name is CodeEye bot.")
                else:
                    chk = random.choice(choce)
                    send("%s, My name is CodeEye bot."%chk)
 
            if "age" in cb:
                send("I am not sure. CodeEye made me immortal.;-)")
            ...
            if "your owner" in cb:
                send("He is none other than Code Eye.")
            ...
            if "you speak" in cb:
                p.click(1210,682)
                eng.say("Just learning the basics with CodeEye. How was that ? ")
                eng.runAndWait()
                p.click(1210,682)
